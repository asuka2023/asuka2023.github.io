...
async function checkEnumerateDevices(selectId) {
    try {
        if (!navigator.mediaDevices?.enumerateDevices) {
            alert("瀏覽器不支援多媒體裝置");
            window.parent.postMessage("Message:瀏覽器不支援多媒體裝置");
            return false;
        } else {
            var devices = await navigator.mediaDevices.enumerateDevices();
             var result = getDevices(devices, selectId);
             if(!result){
                window.parent.postMessage("Message:瀏覽器查無此裝置相機鏡頭");
             }
            return result;
        }
    }
        ...
        window.parent.postMessage(`Message:${err.name}: ${err.message}`);
        return false;
    }
}
    ...
    try {
        ...
        if (!("BarcodeDetector" in globalThis)) {
            console.log("此瀏覽器不支援條碼掃描。");
             window.parent.postMessage("Message:此瀏覽器不支援條碼掃描。");
            return false;

        } else {
            console.log("此瀏覽器支援條碼掃描");
             window.parent.postMessage("Message:此瀏覽器支援條碼掃描");
            return true;
...
async function openCamera(selectId, videoId, focalId) {
      ...
      if (sel.length > 0 && sel.value !== null) {
          ...
          window.parent.postMessage("Message:瀏覽器無法取得相機鏡頭");
      }
            ...
            barcodeDetector
                ...
                .then((barcodes) => {
                    ...
                    if(barcodes.length > 0 && result != ''){
                      window.parent.postMessage(`Barcode:${result}`);
                    }
