...
async function checkEnumerateDevices(selectId) {
  console.log('checkEnumerateDevices');
    try {
        if (!navigator.mediaDevices?.enumerateDevices) {
            console.log('瀏覽器不支援多媒體裝置');
            //alert("瀏覽器不支援多媒體裝置");
            postMessage("Message:瀏覽器不支援多媒體裝置");
            return false;
        } else {
            var devices = await navigator.mediaDevices.enumerateDevices();
              console.log('navigator.mediaDevices.enumerateDevices');
            var result = getDevices(devices, selectId);
              console.log('getDevices(devices, selectId)');
            if (!result) {
                console.log('瀏覽器查無此裝置相機鏡頭');
               // alert("瀏覽器查無此裝置相機鏡頭");
                postMessage("Message:瀏覽器查無此裝置相機鏡頭");

            }
            else {
                result = checkBarcodeSupport();
       ...
       // alert(`${err.name}: ${err.message}`);
        postMessage(`Message:${err.name}: ${err.message}`);
        return false;
    }
}
...
function checkBarcodeSupport() {
    try {
        ...
        if (!("BarcodeDetector" in globalThis)) {
            ...
            postMessage("Message:此瀏覽器不支援條碼掃描。");
            return false;

        } else {
            console.log("此瀏覽器支援條碼掃描");
            //postMessage("Message:此瀏覽器支援條碼掃描");
            return true;
...
async function openCamera(selectId, videoId, focalId) {
    ...
    //設定鏡頭選單元件變數
    var sel = document.getElementById(selectId);
    if (sel.length > 0 && sel.value !== null) {
        //設定開啟相機參數
        var constraints = {
            audio: false,
            video: { frameRate: { min: 10, max: 30 }, deviceId: sel.value, },
        };
        //開啟相機
        try {
        ...
        console.error("瀏覽器無法取得相機鏡頭");
        postMessage("Message:瀏覽器無法取得相機鏡頭");
    }
...
//初始化首頁監聽器
function homeEventListener(homeId){
	//設定首頁按鈕元件變數
    const btn_home = document.getElementById(homeId);
    //監聽按鈕首頁
    btn_home.addEventListener('click', function () {
      postMessage(`home`,100);
        ...
        if (stream != null) {
            ...
            barcodeDetector
                ...
                .then((barcodes) => {
                    ...
                    if (barcodes.length > 0 && result != '') {
                        postMessage(`Barcode:${result}`,100);
                    }
...
// 傳送訊息給其他頁面接收，延遲500毫秒避免其他頁面在LOAD無法接收
function postMessage(message,s = 1000) {
    setTimeout(() => {
